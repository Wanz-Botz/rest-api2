...
const listkey = ["Wanz", "wanzbotz", "WanzBotz", "wanzz"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'WanzBotz') return res.json(loghandler.invalidKey)
...
///NSFW END
router.get('/tiktod', async(req, res) => {
var apikeyInput = req.query.apikey,
url = req.query.url
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
       ...
       fetch(encodeURI(`http://api-dafatrizaa.herokuapp.com/tiktok?url=${url}&apikey=aovEZXU5`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/nulis', async (req, res, next) => {
    ...
    if (!text) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter text"})
       ...
       buffer = await fetch(`https://hardianto.xyz/api/nuliskiri?text=${text}&apikey=hardianto`)
        res.type('png')
        res.send(buffer)
         .catch(e => {
         	res.json(loghandler.error)
...
router.get('/attp', async(req, res, next) => {
  ...
  let hasil = 'https://hardianto.xyz/api/maker/attp?text=${encodeURIComponent(text)}apikey=hardianto'
  data = await fetch(hasil).then(v => v.buffer())
